...
app.get('/team_stats', function(req, res){
	var query = "select football_games.*, (case when home_score > visitor_score then 'CU Boulder' else visitor_name end) as winner from football_games order by game_date";
...
app.get('/player_info/select_player', function(req, res){
	...
	db.task('load-players', task =>
		task.batch([
			task.any("select * from football_players order by id"),
			task.oneOrNone("select * from football_players where id=$1", player_id)
			...
			if(player){
				var games = task.one("select count(*) as games_played from football_games where $1 = any(players)", player.id)
			}
			else{
				var games = null;
			}
			return Promise.all([Promise.resolve(results[0]), Promise.resolve(player), games]);
		})
	).then(query_results => {
